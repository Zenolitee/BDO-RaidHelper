import { ActionRowBuilder, ActivityType, ButtonBuilder, ButtonStyle, ChannelSelectMenuBuilder, ChannelType, Client, Events, GatewayIntentBits, ModalBuilder, PermissionFlagsBits, RoleSelectMenuBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, TextInputBuilder, TextInputStyle } from "discord.js";
import { nanoid } from "nanoid";
import { config } from "./config.js";
import { formatGroupBadge, formatGroupName, getGroupLabel } from "./emojis.js";
import { buildNodeWarTitle, getGroupsForPreset, getNodeWarCapacity, labelTier, labelWarDay, NODE_WAR_PRESETS } from "./nodewar-presets.js";
...
const WIZARD_DAYS = [...WEEKDAYS];
const wizardStates = new Map();
const editWizardStates = new Map();
/**
...
async function listEvents(interaction, store) {
...
function renderEventListCard(event, index, userId) {
...
async function setNodeWarChannel(interaction, store) {
...
async function createTestEvent(interaction, store) {
    const guildId = requireGuildId(interaction);
...
async function showEvent(interaction, store) {
...
async function startEventWizard(interaction, store, createToday = false) {
...
async function handleSelect(interaction, _store, _client) {
    ...
    if (parsed.action === "tier") {
        state.tier = interaction.values[0];
...
async function handleModal(interaction, _store, _client) {
    if (interaction.customId.startsWith("editwizard-")) {
...
async function setSlots(interaction, store, client) {
...
async function startEditWizard(interaction, store) {
    const id = interaction.options.getString("id", true);
    const guildId = requireGuildId(interaction);
...
async function createEditWizardState(store, guildId, userId, id) {
...
async function handleEditWizardSelect(interaction) {
    ...
    if (parsed.action === "category") {
        state.step = interaction.values[0];
    }
...
async function handleEditWizardButton(interaction, store, client) {
    const parsed = parseEditWizardCustomId(interaction.customId);
    if (!parsed) {
        return;
    }
    ...
    if (parsed.action === "confirm") {
        ...
        const announcementScheduleChanged = nextAnnouncementDate !== event.announcementDate || state.announcementTime !== event.announcementTime;
...
function renderEditWizard(state, event) {
        ...
        return {
            ...
            components: [
                new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                    ...
                    .addOptions(WIZARD_DAYS.map((day) => new StringSelectMenuOptionBuilder()
    ...
    if (state.step === "post-time") {
        return {
            content,
            components: [
                new ActionRowBuilder().addComponents(wizardButton(`editwizard-post-time-default:${state.userId}`, "Default 10:15 PM Singapore Time", ButtonStyle.Primary), wizardButton(`editwizard-post-time-custom:${state.userId}`, "Custom post time", ButtonStyle.Secondary), wizardButton(`editwizard-back:${state.userId}`, "Back", ButtonStyle.Secondary))
            ]
        };
    }
    if (state.step === "slots") {
        return {
            content,
            components: [
                new ActionRowBuilder().addComponents(wizardButton(`editwizard-slots:${state.userId}`, "Adjust composition", ButtonStyle.Primary), wizardButton(`editwizard-back:${state.userId}`, "Back", ButtonStyle.Secondary))
            ]
        };
    }
        ...
        return {
            ...
            components: [
                new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                    ...
                    .addOptions(new StringSelectMenuOptionBuilder().setLabel("One-time only").setValue("once").setDefault(state.recurrence !== "weekly"), new StringSelectMenuOptionBuilder().setLabel("Repeat weekly").setValue("weekly").setDefault(state.recurrence === "weekly"))),
                editWizardBackRow(state.userId)
            ]
        };
    }
    return {
        ...
        components: [
            new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .addOptions(new StringSelectMenuOptionBuilder().setLabel("Days").setValue("days"), new StringSelectMenuOptionBuilder().setLabel("Time to post").setValue("post-time"), new StringSelectMenuOptionBuilder().setLabel("Composition roles and slots").setValue("slots"), new StringSelectMenuOptionBuilder().setLabel("Repeat mode").setValue("recurrence"))),
            new ActionRowBuilder().addComponents(wizardButton(`editwizard-confirm:${state.userId}`, "Confirm Update", ButtonStyle.Success), wizardButton(`editwizard-cancel:${state.userId}`, "Cancel", ButtonStyle.Danger))
        ]
    };
}
function buildEditWizardValuesModal(state) {
    return new ModalBuilder()
        .setCustomId(`editwizard-slots:${state.userId}`)
        .setTitle("Edit Event Slots")
        .addComponents(textInputRow("mainball", "Mainball/FFA slots", String(state.mainball)), textInputRow("defense", "Defense slots", String(state.defense)), textInputRow("shai", "Shai slots", String(state.shai)), textInputRow("zerker", "Zerker slots", String(state.zerker)));
}
function buildEditWizardPostTimeModal(state) {
    return new ModalBuilder()
...
function editWizardPrompt(step) {
    return {
...
function editWizardBackRow(userId) {
    return new ActionRowBuilder().addComponents(wizardButton(`editwizard-back:${userId}`, "Back", ButtonStyle.Secondary), wizardButton(`editwizard-cancel:${userId}`, "Cancel", ButtonStyle.Danger));
}
function parseEditWizardCustomId(customId) {
...
function getEditWizardState(userId, actorId) {
    if (userId !== actorId) {
...
function refreshEditWizardTimeout(state) {
    state.expiresAt = Date.now() + WIZARD_TIMEOUT_MS;
}
async function setRecurring(interaction, store, client) {
    const id = interaction.options.getString("id", true);
    const event = await getGuildEvent(store, requireGuildId(interaction), id);
    if (!event) {
        throw new Error("Event not found.");
...
async function deleteEvent(interaction, store) {
    const id = await resolveEventId(store, requireGuildId(interaction), interaction.options.getString("id", true));
    await store.deleteEvent(id);
    await interaction.reply({ content: `Deleted event ${id}.`, ephemeral: true });
}
async function repostEvent(interaction, store, client) {
    const id = interaction.options.getString("id", true);
    const event = await getGuildEvent(store, requireGuildId(interaction), id);
    if (!event) {
        throw new Error("Event not found.");
...
async function handleButton(interaction, store, client) {
    if (interaction.customId.startsWith("editwizard-")) {
    ...
    if (action === "event-post-now") {
        ...
        const channelId = event.announcementChannelId ?? event.channelId ?? (await getNodeWarChannelId(store, interaction.guildId));
        if (!channelId) {
            throw new Error("Set a Node War channel first with /set-nwchannel.");
        }
...
async function handleWizardButton(interaction, store) {
    const parsed = parseWizardCustomId(interaction.customId);
    if (!parsed) {
        return;
    }
...
function wizardStepComponents(state) {
    if (state.step === "tier") {
        return [
            new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                .setCustomId(`wizard-tier:${state.userId}`)
                .setPlaceholder("Select Node War tier")
                .addOptions(tierOption("Tier 1 - Balenos/Serendia", "tier1", state.tier), tierOption("Tier 2 - Calpheon/Ulukita", "tier2", state.tier), tierOption("Tier 3 - Valencia/Edania", "tier3", state.tier)))
        ];
    }
    if (state.step === "days") {
        return [
    ...
    if (state.step === "repeat") {
        return [
            new ActionRowBuilder().addComponents(wizardButton(`wizard-repeat:${state.userId}:once`, "One-time only", ButtonStyle.Secondary), wizardButton(`wizard-repeat:${state.userId}:weekly`, "Repeat weekly", ButtonStyle.Primary))
        ];
    }
    if (state.step === "post-time") {
        return [
            new ActionRowBuilder().addComponents(wizardButton(`wizard-post-time:${state.userId}:default`, "Default 10:15 PM Singapore Time", ButtonStyle.Primary), wizardButton(`wizard-post-time:${state.userId}:custom`, "Custom post time", ButtonStyle.Secondary))
        ];
    }
    if (state.step === "ping") {
        return [
            new ActionRowBuilder().addComponents(new RoleSelectMenuBuilder()
                ...
                .setMinValues(1)
                .setMaxValues(25)),
            new ActionRowBuilder().addComponents(wizardButton(`wizard-ping:${state.userId}:default`, "Use default NODEWAR_ROLE_ID", ButtonStyle.Primary, !config.nodeWarRoleId), wizardButton(`wizard-ping:${state.userId}:none`, "No ping", ButtonStyle.Secondary), wizardButton(`wizard-ping:${state.userId}:continue`, "Continue with selected roles", ButtonStyle.Success, state.pingRoleIds.length === 0))
        ];
    }
    if (state.step === "slots") {
        return [
            new ActionRowBuilder().addComponents(wizardButton(`wizard-slots:${state.userId}:default`, "Use default slots", ButtonStyle.Primary), wizardButton(`wizard-slots:${state.userId}:custom`, "Customize slots", ButtonStyle.Secondary))
        ];
    }
    ...
    return [
        new ActionRowBuilder().addComponents(wizardButton(`wizard-confirm:${state.userId}:confirm`, "Confirm Create", ButtonStyle.Success), wizardButton(`wizard-confirm:${state.userId}:cancel`, "Cancel", ButtonStyle.Danger))
    ];
}
function renderWizardSummary(state) {
    ...
    const capacities = state.tier && state.days.length
        ? state.days.map((day) => `${labelWarDay(day)} ${getNodeWarCapacity(state.tier, day)}`).join(", ")
        : "Not selected";
...
function tierOption(label, value, selected) {
    return new StringSelectMenuOptionBuilder().setLabel(label).setValue(value).setDefault(selected === value);
}
function wizardButton(customId, label, style, disabled = false) {
    return new ButtonBuilder().setCustomId(customId).setLabel(label).setStyle(style).setDisabled(disabled);
}
function cancelRow(userId) {
    return new ActionRowBuilder().addComponents(wizardButton(`wizard-cancel:${userId}`, "Cancel", ButtonStyle.Danger));
}
function buildWizardPostTimeModal(userId) {
    return new ModalBuilder()
...
function buildWizardSlotsModal(state) {
    return new ModalBuilder()
        .setCustomId(`wizard-slots:${state.userId}`)
        .setTitle("Customize Slots")
        .addComponents(textInputRow("defense", "Defense slots", String(state.slots.defense)), textInputRow("zerker", "Zerker slots", String(state.slots.zerker)), textInputRow("shai", "Shai slots", String(state.slots.shai)));
}
function textInputRow(id, label, value) {
...
async function confirmWizard(state, store, createdBy) {
...
function nextStepAfterSlots(state) {
    return state.channelId || config.nodeWarChannelId ? "confirm" : "channel";
}
function buildWizardGroups(state, totalCapacity) {
...
function parseWizardCustomId(customId) {
...
function getWizardState(userId, actorId) {
    if (userId !== actorId) {
...
function refreshWizardTimeout(state) {
    state.expiresAt = Date.now() + WIZARD_TIMEOUT_MS;
}
function parseSlotValue(value, label) {
...
async function resolveEventId(store, guildId, input) {
    const normalized = normalizeEventIdInput(input);
    const event = (await store.listEvents()).find((candidate) => candidate.guildId === guildId &&
...
function validateWizardSlots(state) {
...
async function startNodeWarScheduler(client, store) {
 ...
 */
async function runNodeWarScheduler(client, store) {
...
export async function rollCompletedWeeklyEvents(client, store, now) {
...
function groupsForCapacity(groups, totalCapacity) {
...
function nextSelectedRaidAfter(date, days) {
    return days
...
async function closeExpiredOneTimeEvents(client, store) {
    const events = await store.listEvents();
    const now = Date.now();
    for (const event of events) {
...
async function postDueScheduledEvents(client, store, now) {
    const events = await store.listEvents();
    for (const event of events) {
        ...
        const channelId = event.announcementChannelId ?? event.channelId ?? (event.guildId ? await getNodeWarChannelId(store, event.guildId) : undefined);
        if (!channelId) {
...
function groupCapacity(event, key, fallback) {
    return event.groups.find((group) => group.key === key)?.capacity ?? fallback;
}
async function getNodeWarChannelId(store, guildId) {
    const settings = await store.getSettings();
    return (settings.nodeWarChannelIds?.[guildId] ??
        (guildId === config.discordGuildId ? settings.nodeWarChannelId ?? config.nodeWarChannelId : undefined));
}
async function postEventToChannel(client, channelId, event, roleIds = getAnnouncementRoleIds(event)) {
...
function getAnnouncementRoleIds(event) {
...
export async function refreshEventMessage(client, event) {
...
async function refreshOpenEventMessages(client, store) {
...
function renderEventMessagePayload(event) {
    return {
        embeds: [renderEventEmbed(event, true)],
...
function requireButtonGuildId(interaction) {
    if (!interaction.guildId) {
        throw new Error("Use this button inside a Discord server.");
    }
    return interaction.guildId;
}
function assertListButtonOwner(interaction, userId) {
...
function announcementDateForEvent(eventDate) {
...
function nextWarDateInTimezone(day, timezone) {
...
function todayWarDayFromSelection(days, timezone) {
    const day = days[0];
    if (!day) {
...
function currentWarDay(timezone) {
...
export function nextWarDayFromSelection(days, timezone, now = zonedNow(timezone)) {
...
function nextWarDayAfterToday(timezone, allowedDays = WIZARD_DAYS, now = zonedNow(timezone)) {
    const todayIndex = weekdayIndex(now.weekday);
...
function warDayFromWeekday(weekday) {
